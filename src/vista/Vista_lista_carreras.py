...
class Vista_lista_carreras(QWidget):
    ...
    def mostrar_carreras(self, lista_carreras):
        ...
        if (self.carreras!= None and len(self.carreras)>0) :
            ...
            for dic_carrera in self.carreras:
                ...
                self.distribuidor_tabla_carreras.layout().setRowStretch(0, 1)
    ...
    def terminar_carrera(self, id_carrera):
        ...
        if dialogo.resultado == 1:
            self.hide()
            resultado = self.interfaz.mostrar_reporte_ganancias(dialogo.combobox_competidores.currentData())
            if(resultado == False):
                self.mensaje_generar("Error reporte Ganancias", "No fue posible generar el reporte, verifique si la carrera tiene apuestas.")
                self.show()
    ...
    def eliminar_carrera(self,indice_carrera): 
        ...
        if respuesta == QMessageBox.Yes:
             ...
             if (resultado == False):
                 self.mensaje_generar('Eliminar Carrera', "La carrera no puede tener apuestas asociadas para poder eliminarla")
    
    def mensaje_generar(self, titulo_mensaje, mensaje):
        mensaje_error=QMessageBox()
        mensaje_error.setIcon(QMessageBox.Information)
        mensaje_error.setText(mensaje)        
        mensaje_error.setWindowTitle(titulo_mensaje)
